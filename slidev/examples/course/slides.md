---
theme: slidev-theme-tulip-lab
title: Human-Centred AI Research
subtitle: From research question to responsible evidence
layout: cover
addons:
  - slidev-addon-tulip-lab-live
  - slidev-addon-tulip-lab-pages
course: TULIP Lab Course Example
author: Professor Gang Li
affiliation: Deakin University, Australia
speakerProfileUrl: https://www.tulip.academy/members/gangli/
speakerHighlights:
  - University Thesis Examination Committee
  - Researcher Development Director, Deakin Cyber
  - IEEE technical leadership in data mining, analytics, and enterprise systems
  - Editorial service across tourism, technology, and cyber security venues
contactOrganisation: School of Information Technology
contactAffiliation: Deakin University, Australia
email: director@tulip.academy
website: https://www.tulip.academy
github: https://github.com/tulip-lab
scholar: https://scholar.google.com/citations?user=dqwjm-0AAAAJ&hl=en
aspectRatio: 16/10
canvasWidth: 1280
---

---
layout: toc
navigation: toc
---

<TableOfContents />

---
layout: tulip-speaker
section: TULIP Lab
tocExpand: false
session: Gang Li
---

---
layout: tulip-deakin
session: Deakin
---

---
layout: tulip-deakin-rankings
---

---
layout: tulip-academy
session: TULIP Lab
clicks: 4
---

---
layout: section
section: Foundations
block: 1
---

# Foundations

Start with people, purposes, and evidence.

---
layout: default
session: 1A
sessionTitle: Research Frame
---

::header::

# Research Frame

<div class="tulip-purpose">Turn a broad concern into an answerable question</div>

::default::

<div class="grid grid-cols-3 gap-4">
  <div class="tulip-card tulip-card--blue"><strong>People</strong><br>Who experiences the system?</div>
  <div class="tulip-card tulip-card--green"><strong>Purpose</strong><br>What outcome matters?</div>
  <div class="tulip-card tulip-card--purple"><strong>Evidence</strong><br>What would support the claim?</div>
</div>

---
layout: references
section: References
block: 2
navigation: false
---

# References

1. TULIP Lab. (2026). *Human-centred AI research example*.

---
layout: tulip-questions
section: Closing
tocExpand: false
session: Questions
navigation: false
---

---
layout: tulip-contact
title: Contact
session: Contact
---
