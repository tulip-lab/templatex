...
%\Item
%Arithmetic Mean Of All Elements In Each Feature Can Describe The Features Of The Group.
...
\item
How to \textcolor{orange}{evaluate} the outlying degree
...
%\begin{figure}
  ...
  %\caption{OS Identification}\label{OS-Identification}
%\end{figure}
...
\item
%Reduce the Complexity through a complete
...
\begin{itemize}
\item
%The time complexity of naive method is $O(2^d)$.
Before: $O(2^d)$  \\
Now:    $O(d*n^{2})$

%\item
%In GOAM, the time complexity is only $O(d*n^{2})$.
\end{itemize}
\end{itemize}
...
Data
\begin{itemize}
 \item %a web crawler was deployed to Extract data for all NBA teams until March $30$, $2018$;
...
Data Preprocess
\end{itemize}
%continuous value, \textcolor{orange}{discretize} them.
...
\item
\smallskip
%Extended the task of outlying aspects mining
%to the \textcolor{orange}{group level}
%and formalize the research problem;
Formalize the problem of \emph{Group Outlying Aspects Mining} by
extending outlying aspects mining;

\item
\smallskip
%A novel algorithm named GOAM is proposed to solve
%the \textcolor{orange}{group} outlying aspects mining problem;
Propose a novel method \textcolor{orange}{GOAM algorithm} to solve the
\emph{Group Outlying Aspects Mining} problem;

\item
\smallskip
%Using the pruning strategies to reduce the time complexity;
...
{
\vspace{\stretch{1}}
Miss \emph{Shaoni Wang}\\
Xi'an Shiyou University, Xi'an, China
\begin{description}
 \item[Email] \href{mailto:snwangxyz@gmail.com}
 {\textsc{\footnotesize{snwangxyz@gmail.com}}}
